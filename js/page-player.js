...
function PagePlayer(oConfigOverride) {
  ...
  this.setPageTitle = function(sTitle) {
    ...
    try {
      if(sTitle) sTitle = sTitle.replace(/(<([^>]+)>)/ig, '');
  ...
  this.events = {
    ...
    play: function() {
	  if(self.config.events.onplay) self.config.events.onplay.apply(self);
    ...
    stop: function() {
	  if(self.config.events.onstop) self.config.events.onstop.apply(self);
      pl.removeClass(this._data.oLI,this._data.className);
    ...
    pause: function() {
	  ...
	  if(self.config.events.onpause) self.config.events.onpause.apply(self);
      pl.removeClass(this._data.oLI,this._data.className);
    ...
    resume: function() {
      if (pl.dragActive) return false;
	  if(self.config.events.onresume) self.config.events.onresume.apply(self);
      pl.removeClass(this._data.oLI,this._data.className);
      this._data.className = pl.css.sPlaying;
      pl.addClass(this._data.oLI,this._data.className);
    },
    ...
    finish: function() {
	  if(self.config.events.onfinish) self.config.events.onfinish.apply(self);
      pl.removeClass(this._data.oLI,this._data.className);
      this._data.className = '';
      this._data.oPosition.style.width = '0px';
    ...
    whileloading: function() {
	  if(self.config.events.onwhileloading) self.config.events.onwhileloading.apply(self);
    ...
    onload: function() {
      if (!this.loaded) {
	  	if(self.config.events.onload) self.config.events.onload.apply(self);
	...
	metadata: function() {
	  if(self.config.events.onmetadata) self.config.events.onmetadata.apply(self);
	  // video-only stuff
    ...
    whileplaying: function() {
      var d = null;
      if(self.config.events.onwhileplaying) self.config.events.onwhileplaying.apply(self);
